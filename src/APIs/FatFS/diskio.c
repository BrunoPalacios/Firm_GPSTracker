...
#define	SD_SPI_NUM		0	// SPI0

#define SD_SELECT()     GPIO_ResetBits(GPIO_CS_PORT, GPIO_CS_PIN)       /* CS = L */
#define SD_DESELECT()   GPIO_SetBits(GPIO_CS_PORT, GPIO_CS_PIN)         /* CS = H */
...
static void SPI_Release(void){
	...
	SD_DESELECT();
	SPI_ReadWrite_Byte(SD_SPI_NUM, 0xff);
}
/**
...
static BYTE Wait_Ready (void)
{
    ...
    do{
    	...
    	res = SPI_ReadWrite_Byte(SD_SPI_NUM, 0xFF);
        				 ...
        				 DWORD arg)		/* Argument */
{
    ...
    /* Send command packet */
    SPI_ReadWrite_Byte(SD_SPI_NUM, cmd);
    SPI_ReadWrite_Byte(SD_SPI_NUM, (BYTE)(arg >> 24));		/* Argument[31..24] */
    SPI_ReadWrite_Byte(SD_SPI_NUM, (BYTE)(arg >> 16));		/* Argument[23..16] */
    SPI_ReadWrite_Byte(SD_SPI_NUM, (BYTE)(arg >> 8));		/* Argument[15..8] */
    SPI_ReadWrite_Byte(SD_SPI_NUM, (BYTE)arg);				/* Argument[7..0] */
    n = 0x01;							        /* Dummy CRC + Stop */
    if(cmd == CMD0) n = 0x95;			        /* Valid CRC for CMD0(0) */
    if(cmd == CMD8) n = 0x87;			        /* Valid CRC for CMD8(0x1AA) */
    SPI_ReadWrite_Byte(SD_SPI_NUM, n);
...
static bool Receive_DataBlock(BYTE *buff, UINT btr)			
{
	...
	{							
        token = SPI_ReadWrite_Byte(SD_SPI_NUM, 0xFF);
	...
	{						
        *buff++ = SPI_ReadWrite_Byte(SD_SPI_NUM, 0xFF);

	} while(btr--);

    SPI_ReadWrite_Byte(SD_SPI_NUM, 0xFF);		/* Discard CRC */
    SPI_ReadWrite_Byte(SD_SPI_NUM, 0xFF);
...
static bool Transmit_DataBlock(const BYTE *buff, BYTE token)	
{
    ...
    SPI_ReadWrite_Byte(SD_SPI_NUM, token);	 /* Transmit data token */
	...
	{	            
		...
		{							/* Transmit the 512 byte data block to MMC */
            SPI_ReadWrite_Byte(SD_SPI_NUM, *buff++);

		} while(--wc);

    	SPI_ReadWrite_Byte(SD_SPI_NUM, 0xFF);		/* CRC (Dummy) */
        SPI_ReadWrite_Byte(SD_SPI_NUM, 0xFF);

        resp = SPI_ReadWrite_Byte(SD_SPI_NUM, 0xFF);/* Reveive data response */
...
DSTATUS disk_initialize(BYTE drv)
{
	...
	SD_DESELECT();
	SPI_ReadWrite_Byte(0, 0xFF);

	for(timer1=50; timer1>0; ); //delay 500ms for power on.


	spiSpeed(SD_SPI_NUM, SPI_BaudRatePrescaler_256, SPI_CPOL_High, SPI_CPHA_2Edge); //low speed, 72000/256=281.25KHz
	SD_DESELECT();
    ...
    do
	{
        for(n=10; n>0; n--) SPI_ReadWrite_Byte(SD_SPI_NUM, 0xFF);	    /* 80 dummy clocks */
	...
	{	    
        for(n = 0; n < 4; n++) 
            ocr[n] = SPI_ReadWrite_Byte(SD_SPI_NUM, 0xFF);	/* Get trailing return value of R7 resp */
		...
		{			
			...
			{		
                ...
                for(n = 0; n < 4; n++)
                    ocr[n] = SPI_ReadWrite_Byte(SD_SPI_NUM, 0xFF);
    ...
    spiSpeed(SD_SPI_NUM, SPI_BaudRatePrescaler_16, SPI_CPOL_High, SPI_CPHA_2Edge);
...
DRESULT disk_ioctl(BYTE drv, BYTE ctrl, void *buff)
{
	...
	{	/* Make sure that no pending write process */
        ...
        case GET_BLOCK_SIZE :	
			...
			{			
				...
				{		
					...
					{				
                        for(n = 64 - 16; n; n--) 
							SPI_ReadWrite_Byte(SD_SPI_NUM, 0xFF);	/* Purge trailing data */
        ...
        case MMC_GET_OCR :		
			...
			{	
                for(n = 4; n; n--) *ptr++ = SPI_ReadWrite_Byte(SD_SPI_NUM, 0xFF);
                res = RES_OK;
            }
            break;
